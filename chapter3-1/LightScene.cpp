...
LightScene::LightScene(const char *_title): GLObject(_title) {
    this->enableDeepTest();
}
...
void LightScene::init() {
    ...
    auto pMouseCallBack = [](GLFWwindow *_window, double _xPosIn, double _yPosIn) {
        static_cast<LightScene *>(glfwGetWindowUserPointer(_window))->mouseCallBack(_window, _xPosIn, _yPosIn);
    };
    glfwSetCursorPosCallback(m_window, pMouseCallBack);

    // 绑定鼠标滚动回调事件
    auto pScrollCallBack = [](GLFWwindow *_window, double _xOffset, double _yOffset) {
        static_cast<LightScene *>(glfwGetWindowUserPointer(_window))->scrollCallBack(_window, _xOffset, _yOffset);
    };
...
void LightScene::initShader() {
    m_objShader = new GLShader("../../chapter3-1/obj.vs", "../../chapter3-1/obj.fs");
    m_lightShader = new GLShader("../../chapter3-1/light.vs", "../../chapter3-1/light.fs");
}
