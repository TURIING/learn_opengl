...
MaterialMap::MaterialMap(const char *_title): GLObject(_title) {
    this->enableDeepTest();
}
...
void MaterialMap::init() {
    ...
    auto pMouseCallBack = [](GLFWwindow *_window, double _xPosIn, double _yPosIn) {
        static_cast<MaterialMap *>(glfwGetWindowUserPointer(_window))->mouseCallBack(_window, _xPosIn, _yPosIn);
    };
    glfwSetCursorPosCallback(m_window, pMouseCallBack);

    // 绑定鼠标滚动回调事件
    auto pScrollCallBack = [](GLFWwindow *_window, double _xOffset, double _yOffset) {
        static_cast<MaterialMap *>(glfwGetWindowUserPointer(_window))->scrollCallBack(_window, _xOffset, _yOffset);
    };
...
void MaterialMap::initVaoAndVbo() {
...
void MaterialMap::paint() {
    ...
    m_objShader->setFloat("material.shininess", 64.0f);
...
void MaterialMap::initShader() {
    m_objShader = new GLShader("../../chapter3-4/obj_vs.glsl", "../../chapter3-4/obj_fs.glsl");
    m_lightShader = new GLShader("../../chapter3-4/light_vs.glsl", "../../chapter3-4/light_fs.glsl");
}

void MaterialMap::initTexture() {
    /* 生成纹理1 */
    // 创建纹理对象
