...
PointLight::PointLight(const char *_title): GLObject(_title) {
    this->enableDeepTest();
}
...
void PointLight::init() {
    ...
    auto pMouseCallBack = [](GLFWwindow *_window, double _xPosIn, double _yPosIn) {
        static_cast<PointLight *>(glfwGetWindowUserPointer(_window))->mouseCallBack(_window, _xPosIn, _yPosIn);
    };
    glfwSetCursorPosCallback(m_window, pMouseCallBack);

    // 绑定鼠标滚动回调事件
    auto pScrollCallBack = [](GLFWwindow *_window, double _xOffset, double _yOffset) {
        static_cast<PointLight *>(glfwGetWindowUserPointer(_window))->scrollCallBack(_window, _xOffset, _yOffset);
    };
...
void PointLight::initShader() {
    m_objShader = new GLShader("../../chapter3-5/point_light/obj_vs.glsl", "../../chapter3-5/point_light/obj_fs.glsl");
    m_lightShader = new GLShader("../../chapter3-5/point_light/light_vs.glsl", "../../chapter3-5/point_light/light_fs.glsl");
}
...
void PointLight::setTexturePath(const std::string &_pathDiffuse, const std::string &_pathSpecular) {
    m_texturePath_diffuse = std::move(_pathDiffuse);
    m_texturePath_specular = std::move(_pathSpecular);
}

void PointLight::setCubePos(const std::vector<glm::vec3> &_cubePos) {
    m_cubePos = _cubePos;
}
