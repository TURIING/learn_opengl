...
SpotLight::SpotLight(const char *_title): GLObject(_title) {
    this->enableDeepTest();
}
...
void SpotLight::init() {
    ...
    auto pMouseCallBack = [](GLFWwindow *_window, double _xPosIn, double _yPosIn) {
        static_cast<SpotLight *>(glfwGetWindowUserPointer(_window))->mouseCallBack(_window, _xPosIn, _yPosIn);
    };
    glfwSetCursorPosCallback(m_window, pMouseCallBack);

    // 绑定鼠标滚动回调事件
    auto pScrollCallBack = [](GLFWwindow *_window, double _xOffset, double _yOffset) {
        static_cast<SpotLight *>(glfwGetWindowUserPointer(_window))->scrollCallBack(_window, _xOffset, _yOffset);
    };
...
void SpotLight::paint() {
    ...
    m_objShader->setFloat("light.curOff", glm::cos(glm::radians(12.5f)));
...
void SpotLight::initShader() {
    m_objShader = new GLShader("../../chapter3-5/spot_light/obj_vs.glsl", "../../chapter3-5/spot_light/obj_fs.glsl");
}
...
void SpotLight::setTexturePath(const std::string &_pathDiffuse, const std::string &_pathSpecular) {
    m_texturePath_diffuse = std::move(_pathDiffuse);
    m_texturePath_specular = std::move(_pathSpecular);
}

void SpotLight::setCubePos(const std::vector<glm::vec3> &_cubePos) {
    m_cubePos = _cubePos;
}
