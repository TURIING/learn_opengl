...
#include "SpotLightSoft.h"

SpotLightSoft::SpotLightSoft(const char *_title): GLObject(_title) {
    this->enableDeepTest();
}

SpotLightSoft::~SpotLightSoft() {
    /* 清理并退出 */
...
void SpotLightSoft::init() {
    glfwSetWindowUserPointer(m_window, this);

    // 绑定鼠标移动回调事件
    auto pMouseCallBack = [](GLFWwindow *_window, double _xPosIn, double _yPosIn) {
        static_cast<SpotLightSoft *>(glfwGetWindowUserPointer(_window))->mouseCallBack(_window, _xPosIn, _yPosIn);
    };
    glfwSetCursorPosCallback(m_window, pMouseCallBack);

    // 绑定鼠标滚动回调事件
    auto pScrollCallBack = [](GLFWwindow *_window, double _xOffset, double _yOffset) {
        static_cast<SpotLightSoft *>(glfwGetWindowUserPointer(_window))->scrollCallBack(_window, _xOffset, _yOffset);
    };
...
void SpotLightSoft::initVaoAndVbo() {
...
void SpotLightSoft::paint() {
    ...
    m_objShader->setVec3("light.position", m_camera.getPosition());
    m_objShader->setVec3("light.direction", m_camera.getFront());
    m_objShader->setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
    m_objShader->setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));
    ...
    m_objShader->setFloat("light.constant", 1.0f);
    m_objShader->setFloat("light.linear", 0.045f);
    m_objShader->setFloat("light.quadratic", 0.0075f);
...
void SpotLightSoft::initShader() {
    m_objShader = new GLShader("../../chapter3-5/spot_light_soft/obj_vs.glsl", "../../chapter3-5/spot_light_soft/obj_fs.glsl");
}

void SpotLightSoft::initTexture() {
    /* 生成纹理1 */
    // 创建纹理对象
...
void SpotLightSoft::setTexturePath(const std::string &_pathDiffuse, const std::string &_pathSpecular) {
    m_texturePath_diffuse = std::move(_pathDiffuse);
    m_texturePath_specular = std::move(_pathSpecular);
}

void SpotLightSoft::setCubePos(const std::vector<glm::vec3> &_cubePos) {
    m_cubePos = _cubePos;
}
